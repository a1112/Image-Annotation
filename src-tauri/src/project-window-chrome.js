...
(function (root) {
  'use strict';
  const interactive = 'button,a,input,select,textarea,label,summary,[contenteditable]:not([contenteditable="false"]),[tabindex]:not([tabindex="-1"]),[role="button"],[role="link"],[role="menuitem"],[role="tab"],[role="checkbox"],[role="radio"],[role="switch"],[role="slider"],[role="textbox"],[role="combobox"],[role="option"],[data-no-drag]';
  ...
  function install(win, doc, macos) {
    ...
    function mount() {
      ...
      for (const [name, direction] of Object.entries({ n: 'North', ne: 'NorthEast', e: 'East', se: 'SouthEast', s: 'South', sw: 'SouthWest', w: 'West', nw: 'NorthWest' })) {
        ...
        if (name.length === 2) {
          Object.assign(edge.style, { width: '8px', height: '8px', cursor: name === 'nw' || name === 'se' ? 'nwse-resize' : 'nesw-resize', [name.includes('n') ? 'top' : 'bottom']: '0', [name.includes('w') ? 'left' : 'right']: '0' });
        } else if (name === 'n' || name === 's') {
          Object.assign(edge.style, { left: '8px', right: '8px', height: '4px', cursor: 'ns-resize', [name === 'n' ? 'top' : 'bottom']: '0' });
        } else {
          Object.assign(edge.style, { top: '8px', bottom: '8px', width: '4px', cursor: 'ew-resize', [name === 'w' ? 'left' : 'right']: '0' });
        }
        listen(edge, 'pointerdown', event => {
          if (event.button !== 0 || !canResize(state)) return;
          consume(event);
          // The core command names the direction argument `value`.
          void run('start_resize_dragging', { value: direction });
        });
  ...
  if (typeof module === 'object' && module.exports) module.exports = { dragRegion, canResize, cornerRadius, install };
    ...
    // Init scripts run before the document element exists; DOM access in
    // install() needs the document initialized first. The deferred boot also
    // re-runs after back/forward cache restores, where pagehide already
    // disposed the previous controller.
    const boot = () => install(root, root.document, __PROJECT_CHROME_MACOS__);
