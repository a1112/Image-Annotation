...
class Element extends Target {
  ...
  constructor(value, clickable = false, noDrag = false) { super(); if (value != null) this.attrs.set('data-tauri-drag-region', value); this.clickable = clickable; this.noDrag = noDrag; }
  getAttribute(key) { return this.attrs.get(key) ?? null; }
  setAttribute(key, value) { this.attrs.set(key, value); }
  removeAttribute(key) { this.attrs.delete(key); }
  matches(selector) { return this.clickable || (this.noDrag && String(selector).includes('[data-no-drag]')); }
...
test('interactive elements, shadow paths and explicit opt-out block drag', () => {
  ...
  assert.equal(dragRegion([{}, new Element(null, true), new Element('deep')]), false);
  assert.equal(dragRegion([new Element(null, false, true), new Element('deep')]), false);
  assert.equal(dragRegion([new Element('deep', false, true)]), false);
});
...
test('eight resize edges call native directions and hide after maximizing', async () => {
  ...
  // The native command names the direction argument `value`; guard the IPC contract.
  assert.deepEqual(f.actions().map(c => c.args.value), ['North', 'NorthEast', 'East', 'SouthEast', 'South', 'SouthWest', 'West', 'NorthWest']);
  f.state.maximized = true; f.controller.refresh(); await tick(); await tick();
