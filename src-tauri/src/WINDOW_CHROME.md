...
- `[data-no-drag]` blocks dragging for its subtree, matching the application's
  existing opt-out convention for controls that sit inside a region.
...
From this directory: `node --test project-window-chrome.test.cjs`, or from the
repository root: `npm run test:chrome` (also chained into `npm test`).
From the Tauri crate: `cargo check --locked`.
Manually verify all eight resize directions, drag, double click, native snapping,
fullscreen, keyboard activation of window buttons, close confirmation/tray
behavior, and that annotation gestures near the canvas edges are not captured
by the resize handles, on each supported desktop OS. Automated JavaScript tests
mock native IPC; they do not establish OS-level interaction correctness.
